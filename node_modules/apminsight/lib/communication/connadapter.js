...
function handleConnectResponse(err, uri, responseData) {
    ...
    // Extract and store cluster name if response code is 920 and cluster name is present
    if (!utils.isEmpty(data[constants.s247ClusterName])) {
        apmInsightAgentInstance.getInsInfo().setClusterName(data[constants.s247ClusterName]);
    }
...
function handleDataResponse(err, uri, responseData) {
    ...
    // Extract and store cluster name if response code is 920 and cluster name is present
    if (resCode === constants.agentConfigUpdated && data[constants.s247ClusterName]) {
        insInfo.setClusterName(data[constants.s247ClusterName]);
    }
...
function getConnectPayload(cb) {
    cloudchecker.doCloudCheck(function (instanceName, cloudProvider, isAutoScaleEnabled) {
        var config = apmInsightAgentInstance.getConfig();
        let hostType = utils.isEmpty(cloudProvider)
                    ? os.platform()
                    : cloudProvider;
        if (!utils.isEmpty(config.getExplicitHostType())) {
            hostType = config.getExplicitHostType();
        }
        let isAscEnabled = !!isAutoScaleEnabled;
        if (!utils.isEmpty(config.isAutoScaleEnabled())) {
            isAscEnabled = config.isAutoScaleEnabled(); //overriding with config from agent
        }
        var payload = {
            agent_info: {
        ...
        let processUid = process.env.APMINSIGHT_PROCESS_UID,
            processRuleName = process.env.APMINSIGHT_PROCESS_MONITORING_RULE_NAME;
        if (!utils.isEmpty(processUid)) {
            payload.agent_info[constants.processUid] = processUid;
        }
        if (!utils.isEmpty(processRuleName)) {
            payload.agent_info[constants.processRuleName] = processRuleName;
        }
